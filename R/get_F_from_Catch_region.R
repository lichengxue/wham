...
#' 
#' The get_F_from_Catch_region() function is used to calculate region-specific F given the aggregate region-specific catch
#' Note that some helper functions that support the get_F_from_Catch_region() function are included below.
#' 
#' @param Catch Number of regions 
#' @param NAA n_stocks x n_regions x ny x n_ages; numbers-at-age
#' @param log_M n_stocks x n_regions x ny x n_ages; natural mortality 
#' @param mu n_stocks x n_ages x n_seasons x n_years x n_regions x n_regions; movement rates
#' @param L L: n_years x n_regions; "extra" mortality rate
#' @param sel Model years (or "burn-in" period in MSE)
#' @param fracyr_season n_seasons; length of intervals for each season
#' @param fleet_regions n_fleets; which region each fleet is operating
#' @param fleet_seasons n_fleets x n_seasons; 0/1 indicating whether fleet is operating in the season
#' @param can_move n_stocks x n_seasons x n_regions x n_regions; 0/1 determining whether movement can occur from one region to another
#' @param mig_type n_stocks; 0 = migration after survival, 1 = movement and mortality simultaneous
#' @param waacatch n_fleets x n_ages; weight-at-age 
...
get_F_from_Catch_region <- function(Catch, NAA, log_M, mu, L, sel, fracyr_season, fleet_regions, fleet_seasons, can_move, mig_type, waacatch, trace, F_init) {
  ...
  for (i in 1:(n - 1)) {
    ...
    grad_log_catch_at_F <- numDeriv::grad(func = function(log_F) log_catch_fleets_F_multi(log_F, NAA, log_M, mu, L, sel, fracyr_season, fleet_regions, fleet_seasons, can_move, mig_type, waacatch, trace), x = log_F_i)
