...
#'
#' @export
read_asap3_dat2 <- function(filename, add_years){
  ...
  single_stock_fun = function(filename){
    ...
    # if(!is.null(add_years)) dat$n_years <- dat$n_years + add_years
    ...
    if(!is.null(add_years)) {
      tmp = t(sapply(1:add_years, function(x) tail(dat$M,1)))
      dat$M = rbind(dat$M,tmp)
      tmp = t(sapply(1:add_years, function(x) tail(dat$maturity,1)))
      dat$maturity = rbind(dat$maturity, tmp)
      for(i in 1:length(dat$WAA_mats)){
        tmp = t(sapply(1:add_years, function(x) tail(dat$WAA_mats[[i]],1)))
        dat$WAA_mats[[i]] = rbind(dat$WAA_mats[[i]],tmp)
      }
    }
    ...
    if(!is.null(add_years)) {
      for(i in 1:length(dat$sel_block_assign)){
        dat$sel_block_assign[[i]] <- c(dat$sel_block_assign[[i]],rep(tail(dat$sel_block_assign[[i]],1),add_years))
      }
    }
    ...
    if(!is.null(add_years)) {
      for(i in 1:length(dat$CAA_mats)){
        tmp = c(rep(1/dat$n_ages,dat$n_ages),1)
        tmp = t(sapply(1:add_years, function(x) tmp))
        dat$CAA_mats[[i]] = rbind(dat$CAA_mats[[i]],tmp)
      }
      for(i in 1:length(dat$DAA_mats)){
        tmp = rep(0,dat$n_ages+1)
        tmp = t(sapply(1:add_years, function(x) tmp))
        dat$DAA_mats[[i]] = rbind(dat$DAA_mats[[i]],tmp)
      }
      for(i in 1:length(dat$prop_rel_mats)){
        tmp = rep(0,dat$n_ages)
        tmp = t(sapply(1:add_years, function(x) tmp))
        dat$prop_rel_mats[[i]] = rbind(dat$prop_rel_mats[[i]],tmp)
      }
    }
    # print(ind)
    ...
    if(!is.null(add_years)) {
      for(i in 1:length(dat$IAA_mats)){
        tmp1 = (dat$year1+dat$n_years):(dat$year1+dat$n_years+add_years-1)
        tmp2 = rep(dat$IAA_mats[[i]][dat$n_years,2], add_years)
        tmp3 = rep(dat$IAA_mats[[i]][dat$n_years,3], add_years)
        tmp4 = rep(1/dat$n_ages,dat$n_ages)  
        tmp4 = t(sapply(1:add_years, function(x) tmp4))
        tmp5 = rep(dat$IAA_mats[[i]][dat$n_years,ncol(dat$IAA_mats[[i]])], add_years)
        tmp = cbind(tmp1,tmp2,tmp3,tmp4,tmp5)
        dat$IAA_mats[[i]] = rbind(dat$IAA_mats[[i]],tmp)
      }
    }
    ...
    if(!is.null(add_years)) dat$recruit_cv = c(dat$recruit_cv, rep(tail(dat$recruit_cv,1),add_years))
    ...
    if(!is.null(add_years)) {
      tmp = t(sapply(1:add_years, function(x) tail(dat$catch_cv,1)))
      dat$catch_cv = rbind(dat$catch_cv,tmp)
      tmp = t(sapply(1:add_years, function(x) tail(dat$discard_cv,1)))
      dat$discard_cv = rbind(dat$discard_cv,tmp)
      tmp = t(sapply(1:add_years, function(x) tail(dat$catch_Neff,1)))
      dat$catch_Neff = rbind(dat$catch_Neff,tmp)
      tmp = t(sapply(1:add_years, function(x) tail(dat$discard_Neff,1)))
      dat$discard_Neff = rbind(dat$discard_Neff,tmp)
    }
